...
| 2026-09-20 (the mind-map pass) | Shared (`domain/canvas/Tree.kt`, `ui/canvas/TreeGrammar.kt`, `CanvasScreen.kt`, `CanvasViewModel.kt`, `MindMap.kt`, the sync record, the export); **schema v23** migrates in place on the desktop's DB (`canvas_nodes.parentId`, `folded`, `structure`; `page_canvases.structure`); nothing desktop-only. The desktop's right-click on a card opens the node's menu; Tab and Enter with a selected card add a child and a sibling. Verified on the board: see `docs/critiques/mind-map-function.md`. | §8 |
