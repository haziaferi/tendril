...
| 2026-09-18 (the phone audit) | A document PR; nothing in code. Two of its findings touch shared surfaces the desktop shares: a Touch type scale (T·P1) would live beside `DensityProfile` in `Type.kt` and leave the pointer profiles at 14 / 12.5; `TendrilMenuItem` on both profiles (T·P2) would make the audit's-fixes menu row the phone's too. `docs/critiques/phone-*-full.md`. | §8 |
| 2026-09-18 (the phone's second fixes) | Shared. The desktop is untouched in effect: `touchTypography` applies only under the Touch profile (a desktop never runs it), `barControlHeight()` returns 28 under a pointer, `TendrilMenuItem`'s pointer branch is unchanged, the Calendar's and Tasks' wide branches keep L5's bar (the phone now shares `ViewMenuButton` and the pane header with them; `LayersMenuItems` extracted for both). `docs/critiques/phone-fixes-2-measured.md`. | §8 |
| 2026-09-18 (item 23's Lows) | Shared; three reach the desktop in effect (`docs/critiques/phone-lows-measured.md`): every chrome text's box is its style's line height (`chromeStyle`'s `LineHeightStyle` — the pitches on their `heightIn(min)` floors did not move: rows 28 px at a 931-px-tall window, menus 29); the Postpone slide-over's unit is a `days ▾` menu (the recorded small-things-III #3 closes); a block field no longer writes on a selection-only change (a double-click had captured a revision); the task pane's *When* / *Deadline* as *ven 25*. Android-only: `MainActivity.attachBaseContext` applies the shell's scale to the activity's density — the desktop's windows inherit the composition's density and `WorkbenchEnvironment` keeps scaling there (`platformScaled` false). | §8 |
| 2026-09-18 (items 9 and 20) | `tendril-spec.md` now sits at the repository root (was `Tendril android/`); this file's bare references were already right, `settings.gradle.kts`'s note updated. **The AI key file is DPAPI-wrapped**: `src/main/kotlin/com/tendril/desktopapp/DpapiWrap.kt` (`Crypt32Util.cryptProtectData` / `cryptUnprotectData`, `jna-platform` 5.6.0 already here for the tray; `SecretWrap.None` off Windows), passed to `FileAiKeyStore` by `DesktopAppContainer`; the file `~/.tendril-desktop-dev/anthropic.key` reads `dpapi:` + base64 — a bare key from before is rewritten on the first launch; the NTFS-ACL caveat of 2026-09-13 stands only as the second fence. | §8 |
| 2026-09-18 (item 7 — one canvas at two sizes) | Shared; the desktop's canvas block in a page now draws the board's layer at the fit (`CanvasLayer`, `canvasEmbedHeightDp`), the thumbnail renderer gone; the board itself unchanged in effect (`CanvasBoard` composes the same layer with its interaction). Walked on *Escape test* with *Garden plan* as a block: 326 px tall at the user's window, the three cards and the arrow, a click arms the board. Three desktop Lows recorded in §0.10 item 24 (S1–S3: the strip's ISO date cell, two slide-overs' 47 px rows, the field's unthemed context menu). | §8 |
| 2026-09-18 (item 15 — frames and canvas templates) | Shared. The desktop canvas gains frames (`+ → Frame`, the label the handle, the corner handle, *Delete this frame? Its cards stay.*), *Save as template* in the canvas's pane menu before the workspace's items, and canvas templates in the New sheet. **Two older desktop defects fixed on the walk**: the board's pan moved one delta per drag (`pan` captured in `pointerInput(Unit)`, since the board was built) and the arrow-tap detector consumed every drag — the board pans by the mouse now. `docs/critiques/canvas-frames-function.md`. | §8 |
| 2026-09-19 (item 6 — JSON Canvas; the desktop's Markdown export) | **The desktop gains *Export as Markdown*** in Settings (`DesktopSettingsScreen.MarkdownExportSection`: a `JFileChooser` save dialog, the `jvmCommon` `MarkdownExporter` over a `FileOutputStream`) — Android had it since step 6; the exporter now writes every canvas as a `.canvas` file (JSON Canvas) and counts them in the sentence. Verified: the desktop's zip dropped into an Obsidian vault opened *Garden plan* with its cards, frame and arrow. `docs/critiques/json-canvas-function.md`. | §8 |
| 2026-09-19 (item 3 — measurable habits) | Shared; schema **v21** migrates the desktop database in place (verified on launch: `user_version` 21, the four columns). The Habits list's `+` disc at the pointer's 28 dp, the pane's *Counts* row and chips (*+ 10* · *Undo the last one* · *Log an amount…*), the Add habit slide-over's *Counts something* fields — walked with *Pages read*. `docs/critiques/measurable-habits-function.md`. | §8 |
| 2026-09-19 (item 4 — the check-in) | Shared; schema **v22** migrates the desktop database in place (verified on launch: `user_version` 22, `check_ins`). The row in the workspace pane at the pointer's 28 dp pills, the slide-over *How you've been* — walked. `docs/critiques/check-in-function.md`. | §8 |
| 2026-09-19 (items 16 and 17 — closed) | No code. Item 16 recorded, not built; item 17 closed with `docs/agent-over-export.md` (the desktop's *Export as Markdown* is the export it names). Item 19 decided — built next. | §8 |
| 2026-09-19 (item 19 — the block selection) | Shared; nothing desktop-only. The marquee, the group drag and the keys are the pointer profile's; the F1 card gains *Editor*. Walked on the dev window (`docs/critiques/block-selection-function.md`). | §8 |
| 2026-09-19 (the desktop's measurement reference) | Desktop-only: `WindowsTextScale.kt` (JNA `Advapi32Util` over `HKCU\Software\Microsoft\Accessibility\TextScaleFactor`, absent → 100 %; not Windows → 1.0), `MainWindowActions.systemTextScale` refreshed by a `WindowFocusListener` on the main window, `LocalSystemTextScale` provided in `Main.kt`, `PopOutWindows.kt` and `QuickAddWindow.kt`; the Density section's line says what the reference is. Shared: the rule and the local in `ShellScale.kt`, the read in `WorkbenchEnvironment`. Verified: the chrome pixel-identical across three window sizes; the registry value absent on this machine reads as 100 % (the slider's own change is the user's to try — a system setting is not mine to move). | §8 |
| 2026-09-20 (the rail's labels) | Shared (`Shell.kt`): the rail passes `body` to `ShellItem`, the bar its default `caption`. Measured on the dev window: the label's cap 7 → 10 px, Notion's and the Claude app's. | §8 |
| 2026-09-20 (the pointer type scale) | Shared (`Type.kt`'s `pointerTypography`, `WorkbenchEnvironment`); nothing desktop-only. Measured on the dev window beside Notion and the Claude app: `docs/critiques/desktop-type-grounds.md`. | §8 |
| 2026-09-20 (the size count) | Shared: `pointerTypography`'s `titleLarge` at 16, `CANVAS_CONTENT_MIN_SCALE` = 12.5 ÷ 14 with `ReadableLabels` writing a scaled drawing's words at `caption` below it (the canvas embed and the mind-map card). The desktop page carries three chrome sizes; measured in `docs/critiques/desktop-type-grounds.md`. | §8 |
| 2026-09-20 (the canvas card) | Shared (`Frames.kt`, `CanvasScreen.kt`); nothing desktop-only. The board's card measured 191 × 46 px at its fit; the hovered badges stacked at the right edge. On the user's look: the overlay's frame label and the arrowhead fixed before the merge (the Android spec's row). | §8 |
| 2026-09-20 (the mind-map pass) | Shared (`domain/canvas/Tree.kt`, `ui/canvas/TreeGrammar.kt`, `CanvasScreen.kt`, `CanvasViewModel.kt`, `MindMap.kt`, the sync record, the export); **schema v23** migrates in place on the desktop's DB (`canvas_nodes.parentId`, `folded`, `structure`; `page_canvases.structure`); nothing desktop-only. The desktop's right-click on a card opens the node's menu; Insert and Enter with a selected card add a child and a sibling (Tab never reaches the board — AWT keeps it for focus traversal; the menu's chip reads *Insert*). Verified on the board: see `docs/critiques/mind-map-function.md`. | §8 |
| 2026-09-20 (small things IV) | Shared, one desktop-facing piece: `shared/src/desktopMain/…/PlatformTextMenus.desktop.kt` provides `LocalContextMenuRepresentation` (the register's `surface` / `onSurface` / `surfaceVariant`) and `LocalLocalization` (Cut · Copy · Paste · Select all) for every window through `WorkbenchEnvironment` — the field's right-click menu had been Compose's light default with the OS's Italian labels since 14d. The rest shared (`TendrilMenuItem` in two sheets, the shell's inset, the bar's Journal date, the strip's check, the tree's Tab). Verified on the board: `docs/critiques/small-things-4-function.md`. | §8 |
| 2026-09-20 (the card's width) | Shared (`domain/canvas/Frames.kt`, `Tree.kt`, `CanvasScreen.kt`, `CanvasViewModel.kt`, `MindMap.kt`, `CanvasBlock.kt`, the export); nothing desktop-only. The desktop: the width handle on a selected card's left edge (a pointer's); the optimizer run and Xmind's measure are in `docs/critiques/card-width-optimizer.md`. | §8 |
| 2026-09-20 (S8 — sibling order by drag) | Shared (`Tree.kt`, `CanvasScreen.kt`, `CanvasViewModel.kt`); nothing desktop-only. | §8 |
| 2026-09-20 (S10 — the edit-habit sheet) | Shared; the desktop's habit pane gains the *Edit…* chip. | §8 |
| 2026-09-21 (habits as brush-strokes) | Shared (`HabitStrokes.kt`, `HabitStrokeLayer.kt`, the two grids); nothing desktop-only. | §8 |
| 2026-09-21 (S9 — Timeline and Fishbone) | Shared (`Tree.kt`, `CanvasScreen.kt`); nothing desktop-only. | §8 |
| 2026-09-21 (S11 — the Touch gutter) | Touch only; the desktop's 8 dp margin unchanged. | §8 |
| 2026-09-21 (S12 — collapsible frames) | Shared; the desktop gains the frame's right-click menu and the pill's chevron. | §8 |
| 2026-09-21 (S13 — a hue for a database and a card) | Shared; v24 migrates in place (verified). | §8 |
...
| 2026-09-22 (the fourth verb — Mind map) | Shared (`domain/ai/`, `AiResultSheet.kt`, `PageDetailScreen.kt`); nothing desktop-only. Walked on the desktop: the slide-over, a genuine 401, the key file written and cleared. | §8 |
| 2026-09-22 (syntax highlighting; CLAUDE.md) | Shared (`domain/code/`, the span transformation, `jvmCommon`'s exporter); nothing desktop-only. Walked on the desktop (Kotlin, measured). | §8 |
| 2026-09-22 (the widget readout) | Android only (widgets); nothing shared or desktop changed. | §8 |
| 2026-09-22 (block-level search — v25) | Shared; **v25 migrates in place** on the desktop (`MIGRATION_24_25`, verified 24 → 25 with the heal's rows). `DesktopAppContainer` passes `blockFtsDao()`. | §8 |
| 2026-09-22 (audit: a merge re-arms and publishes what it changed) | Anti-drift entry (§0): `shared/` — `SnapshotMergeResult` gains `touchedEntryIds`, the local ids a pass wrote, and `SnapshotSyncOrchestrator` fills it at the entry and reminder write sites. Additive with a default, so no existing construction site changes. This side does not read it: the desktop has no Calendar Provider analog (§1) and its scheduler re-plans the whole horizon in one call, so `rearmReminders` → `replan()` after a pass is already proportional. Android reads it to drive its coordinator per changed row. | §8 |
| 2026-09-22 (audit: the merge re-plans its reminders) | Anti-drift entry (§0): the same §9.7 seam on this side. `DesktopReminderScheduler.replan()` was reachable from the five `EntryScheduleCoordinator` callbacks and once at startup, and a folder merge passes neither — so a reminder set on the phone arrived here unplanned until the next launch. `SyncBar` and `App` now take a required `rearmReminders`, threaded from `main()` where the scheduler lives, called after a successful pass. No desktop file other than `Main.kt` changed, and nothing shared. Recorded because the parent spec's row for this day's seam work covers the phone's four paths; this is the fifth. | §8 |
| 2026-09-22 (audit: an ordinal `BYDAY` that can only limit) | Anti-drift entry (§0): `shared/` — `RecurrenceSpec.parse` returns null for an ordinal `BYDAY` beside `BYMONTHDAY` or under DAILY/WEEKLY, so such a rule shows its first occurrence alone instead of every `BYMONTHDAY` date (`FREQ=MONTHLY;BYMONTHDAY=13;BYDAY=1FR` expanded to all twelve 13ths). No desktop file changed; the desktop Calendar reads the same expander, so the phantom occurrences were drawn there too. Parent spec §4.1.1 amended. | §8 |
| 2026-09-22 (audit: a stale comment about the desktop's scheduler) | Anti-drift entry (§0): `shared/`'s `WorkbenchCore` KDoc claimed this side's `entryScheduleCoordinator` is a no-op. It has been the real `DesktopReminderScheduler` since B§13.6 #7; the named `NoOpEntryScheduleCoordinator` is gone from the tree. Comment only, no behaviour change. | §8 |
| 2026-09-22 (audit: the arrow editor wrote from a stale snapshot) | Anti-drift entry (§0): `shared/` — `CanvasEdgeDao` gains `getById` (a `@Query`; no schema version, so v25 is unchanged on this side), and `CanvasViewModel.cycleEdgeDirection` / `setEdgeLabel` re-read through it instead of writing back the `CanvasEdge` the arrow sheet was opened with. This side is affected exactly as the phone is: the canvas screen is shared, the desktop reaches it through `WorkbenchScaffold` and its pop-out windows, and the reverted-direction bug was reachable here too. `CanvasScreen` also resolves the open sheet's edge against the live list so the direction label updates. Worth recording on this side for a second reason: the trace that found it was checking `tendril-spec.md` §3.7's claim that Canvas is *not* available on desktop — a claim **this file has contradicted since 2026-09-11** and nothing reconciled. §3.7 now carries the correction and points here. | §8 |
| 2026-09-23 (audit 1.5, 1.8: two desktop-only defects, walked then fixed) | Desktop only; nothing shared or Android changed, so `tendril-spec.md` carries no row. **1.5** — `SyncBar` emitted `syncError?.let { Text(…) }` twice, once inside its `Column` and once as a sibling after it closed, so every sync error was drawn on two consecutive lines. The copy inside the `Column` is kept and takes the stray's bottom padding, leaving the spacing unchanged. **1.8** — the quick-add popup's one-write guard read `added`, which is assigned *inside* the launched coroutine after `quickAddEntry` returns from the database; both Enters of a double tap arrive in that lambda inside the round trip, so both saw null and both wrote. A `writing` flag set in the lambda on the UI thread guards it now, and `added` keeps its own job as the *Added* flash — a write **started** is not a write **completed**. Both were hypotheses in `docs/audit-2026-09-22.md` from the first `--full` run and stayed that way because this module has no test source set; both were walked on the dev build, and both fixes re-walked on the same harness (1.5: two lines → one; 1.8: five double-Enters writing 10 entries → 5). Pinned by the walk, not by a test — the standing case for giving this module tests. | §8 |
| 2026-09-23 (a test source set for this module, and the two defects it found in its first hour) | `src/test/kotlin` exists, with `junit`, `kotlinx-coroutines-test` and `mockk` — every version one the offline Gradle cache already holds. This module had never had a test, and the cost is on the record: rows 1.5, 1.6 and 1.8 all sat as hypotheses through five audits because nothing here could execute a check. `DesktopReminderSchedulerTest` (5 cases) found two defects immediately. **1.6** — `replan()`'s unsynchronised `job?.cancel(); job = launch` shows one firing twice; reproduced in **9 of 25** runs where a hand walk could not reproduce it at all. **New** — `loop()` showed only `pending().first()` and re-planned, but `entryFirings` emits only firings still ahead, so a second firing due in the same minute was dropped *permanently*. Both fixed: `@Synchronized` on `replan()`, the show decision moved into a `fire()` that tests and records `fired` atomically (cancellation is cooperative — a loop back from its `delay` has no suspension point left), and `loop()` now holds its planned batch across the delay. **What this source set does not reach:** Compose UI. The desktop `ui-test-junit4` artifact is not in the offline cache, so the drawing halves of 1.5 and 1.8 stay pinned by the walk. It makes the scheduler gated, not the window. | §8 |
| 2026-09-23 (audit 2.11: the JetBrains Runtime is preferred, not required) | Desktop and CI only; nothing shared or Android changed, so `tendril-spec.md` carries no row. L5 registered the JBR as a hard toolchain requirement in two places — `kotlin { jvmToolchain { vendor = JETBRAINS } }` and the `compose.desktop` `javaHome`. The second resolves with `.get()` at **configuration** time, so it is computed on every invocation of every task in this project and not only by `run` and `packageMsi`, which are the only two that need it. On a machine with no JBR that is a hard configuration failure, and this module accordingly did not build on CI once between 2026-09-17 and 2026-09-23. It went unseen because `build.yml` had been failing to *start* since 09-12 over billing: a hundred consecutive red runs, none of which had executed a step, and the live fault arrived in the middle of them. **The requirement is now what L5's prose always said it was.** The launcher is resolved once, with `runCatching` (a toolchain provider with no match throws on query rather than yielding null); where it resolves, both sites pin the JetBrains vendor exactly as before; where it does not, both fall back to any JDK 21 and the window keeps the OS title bar — L5's own stated fallback. `gradle.properties` keeps its machine-specific path, which is what makes the JBR resolve here, with a comment saying so and saying that the warning it logs on a non-Windows Gradle is expected rather than a fault. Verified by `gradlew -q javaToolchains` still detecting JetBrains JDK 21, and by the desktop suite executing 5/0 with the XML re-written rather than reported UP-TO-DATE. | §8 |
| 2026-09-23 (audit 1.8: the one-write guard is a function, not a lambda — and the reason it could not be tested had expired) | Desktop only; nothing shared or Android changed, so `tendril-spec.md` carries no row. The quick-add popup's one-write guard moves out of `QuickAddWindow`'s Enter lambda into `QuickAddGate.kt`: `QuickAddWriteGate.start()`, true exactly once per instance, claimed on the caller's thread **outside** `launch`, with `launchQuickAdd` carrying the write, the flash and the close. **No behaviour changed** — same branch, same order, same 700 ms, and a null write (a blank title) still claims the gate and still closes. The flag stops being Compose state, which it never needed to be: nothing drew from it, so `mutableStateOf` bought a recomposition per write and no correctness. **Why now.** The row above says 1.8 is “pinned by the walk, not by a test”, and the source-set row says the “drawing halves of 1.5 and 1.8 stay pinned by the walk”. The first stopped being necessary when this module got tests that morning; the second was never quite right, because 1.8 has no drawing half — a flag is not a `Text`. A sweep of the audit for justifications that had quietly expired found both, with four more elsewhere. `QuickAddGateTest` (4 cases) pins it: the second Enter arriving with the first write provably in flight (a `CompletableDeferred`, so no timing assumption — the walk needed real double-taps 0–3 ms apart and this needs none), a fresh open writing again so the first cannot pass vacuously, the blank line, and the claim happening before the dispatcher runs at all — that last one fails if the guard is ever moved inside `launch`. Mutant killed: dropping the early return gives `expected:<1> but was:<2>`, the walk's own duplicate pair. Desktop suite 8 → 12. **1.5 stays walk-pinned**, now for a reason that does not expire: it is a duplicated `Text`, and the desktop `ui-test-junit4` artifact is not in the offline cache. | §8 |
| 2026-09-23 (audit 1.17: a sync pass is a function, not a click handler) | Desktop only; nothing shared or Android changed, so `tendril-spec.md` carries no row. What a sync pass decides now lives in `SyncPass.kt`'s `runSyncPass` — read and merge, refuse to write on a passphrase mismatch, write and re-plan otherwise, and turn a throw into a message — leaving `SyncBar`'s `onClick` holding only the two pieces of state the bar draws (`syncError`, and `syncing` in its `finally`). **No behaviour changed**; the branch order, the messages and the re-plan are the same lines in the same sequence. What changed is reachability: the re-plan that closed 1.17 was pinned by the compiler and an owed two-device walk, because it sat inside a click lambda and the desktop `ui-test-junit4` artifact is not in the offline cache. `SyncPassTest` now covers the clean pass, the mismatch — which must **not** write and must **not** re-plan, the case a "was it called?" test would pass without — and a throwing store. Deleting `rearmReminders()` fails the first, so the assertion pins the fix. Along the way the module's existing scheduler test was found to be flaky under load and made to wait on the firing rather than on the clock (audit 2.14). Desktop suite 5 → 8 tests. | §8 |
| 2026-09-24 (audit 1.1: a root page called CLAUDE broke the Markdown export) | Anti-drift entry (§0): `shared/` — `jvmCommon`'s `MarkdownExporter` reserves its own root `CLAUDE.md` in the per-directory collision set, so a root page titled `CLAUDE` (any case) becomes `CLAUDE (2).md` instead of throwing a `ZipException` that failed the whole export, or writing a twin that extraction merges. This side's *Export as Markdown* (`DesktopSettingsScreen`) builds the same class, so it had the same defect and takes the same fix; nothing desktop-only changed. `tendril-spec.md` §0.10 item 17 carries the amendment. | §0 |
| 2026-09-24 (audit 1.2, 1.3: two writes outside View-Only) | Anti-drift entry (§0): `shared/` — `PageDatabaseViewModel.setHue` now checks the lock, and `PageDetailViewModel.undo`/`redo` check it before moving the undo stack (a refused undo had spent its step). This side runs the same ViewModels, so Ctrl+Z under View-Only had the same defect here, and takes the same fix; nothing desktop-only changed. `tendril-spec.md` §3.1.2 carries the amendment. | §0 |
| 2026-09-24 (audit 1.4: Sync-to-Tasks armed nothing) | Anti-drift entry (§0): `shared/` — `DatabaseSyncManager` takes the `EntryScheduleCoordinator` as a required parameter (`DesktopAppContainer` passes its `scheduler`) and reports every Entry `enableSync`/`bindProperty` writes. Here that is `DesktopReminderScheduler.replan()`, which was never called for a sync-on, so its reminders waited for the loop's next wake — up to its fifteen-minute ceiling — and a firing due before then passed silently. Each report cancels the previous loop, so a bulk sync-on costs one replan. The Android-only half (1.5, an exception row's series) does not apply: `replan()` reads every entry and exception. `tendril-spec.md` §9.7 carries the amendment. | §0 |
| 2026-09-24 (audit 1.6: an .ics import armed the occurrences it had just skipped) | Anti-drift entry (§0): `shared/` — `IcsImporter` reports a recurring base after its EXDATE skip rows exist, not before. This side's .ics import (the Calendar's `···`) runs the same importer, so the `replan()` it triggered could read the series before its skips existed and hold a skipped occurrence until the loop next woke and re-read (at most its fifteen-minute ceiling); nothing desktop-only changed. `tendril-spec.md` §3.2 carries the amendment. | §0 |
| 2026-09-25 (audit 5a.2: one added row claimed an edit to every row) | Anti-drift entry (§0): `shared/` — a row joining a Sync-to-Tasks database (`addRow`, or a labelled page through `LabelMembership`) now goes through `DatabaseSyncManager.addRowToSync`, which seeds its task and writes nothing else, instead of `enableSync`, whose commit bumped the database page and every row. This side runs the same ViewModel and `LabelMembership`, so adding a row on the desktop could overwrite a cell edited on the phone and not yet synced, and the reverse; it takes the same fix, and nothing desktop-only changed. §3's merge granularity is untouched — still whole-page LWW, still the open decision. `tendril-spec.md` §9.4 carries the amendment. | §0 |
...
**The card's width 2026-09-20** — a card as wide as its text between 60 and 220 dp (the optimizer's choice over the user's texts, Xmind measured), a hairline, a width handle. **Small things IV 2026-09-20** — the field's context menu in the register's colours and the app's words; the sheets' rows, the bar's Journal date. **The mind-map pass 2026-09-20** — the tree on the canvas (v23): a parent link, a structure per board and per subtree applied by Tidy, three levels, following frames, folding, headless coloured branches, relationships routed from the outer sides; one grammar for the canvas and the outline's map. **The canvas card 2026-09-20** — a 200 × 48 dp strip that grows a line at a time (Obsidian's proportion). **The size count 2026-09-20** — three sizes on a page (12.5 · 14 · 16), the bar's title at 16, scaled text readable or absent. **The pointer type scale 2026-09-20** — the four small styles one size up beside Notion's and the Claude app's one-size chrome. **The rail's labels at `body` 2026-09-20** — Notion's and the Claude app's 10 px. **The measurement reference shipped 2026-09-19** — the desktop scales by Windows' text size, never by the window. **Item 19 shipped 2026-09-19** — the block selection (Esc, Shift+↑↓, a marquee, a group drag) and the block undo (Ctrl+Z/Y over block operations, the snackbar's Undo); the F1 card's *Editor* group. **Items 16 and 17 closed 2026-09-19** — no code: database-level history recorded until it is missed; the agent's interface is the export (`docs/agent-over-export.md`). **Item 4 shipped 2026-09-19** — the check-in (v22): words on the Journal's day, the month as a sheet. **Item 3 shipped 2026-09-19** — measurable habits (v21). **Item 6 shipped 2026-09-19** — canvases as `.canvas` files in the Markdown zip; the desktop's Markdown export. **Item 15 shipped 2026-09-18** — frames and canvas templates; the board pans at last. **Item 7 shipped 2026-09-18** — one canvas at two sizes. **Items 9 and 20 shipped 2026-09-18** — the spec at the root; the key file DPAPI-wrapped. **Item 23's Lows shipped 2026-09-18** — the phone audit's twelve Lows; §0.10 item 23 closed. **The phone's second fixes shipped 2026-09-18** — the Touch type scale, one Touch row, the phone's chrome folded as L5's, 48 dp targets, one menu row on both profiles. **The phone audit ran 2026-09-18** — three passes over every phone surface beside the four apps on the phone; one High (the Touch type scale), five Med, the decisions listed (§0.10 item 23). **The phone's fixes shipped 2026-09-18** — every sheet scrolls (rule 20), a Touch submenu pushes a level, the canvas's `···`, the rows' meta, a layout test. **The phone catch-up ran 2026-09-18** — the 25 pending phone halves walked; the phone had crashed on launch since 14h·2 (a Touch-only padding), fixed; five findings to a fix PR, then the phone audit (§0.10 item 23). **L7b shipped 2026-09-18** — the quick-add tokens tinted in the line on `findSoft`, the chips kept (Todoist's, measured); **§0.10 item 22 is closed**. **Small things III shipped 2026-09-18** — L11–L14, F9–F14; item 22's Med/Low list closed. **The database's views shipped 2026-09-18** — F4–F8: *Delete view…* asks, the ▾ on the active chip, a new view makes the property it plots by (Notion's), the New view sheet, one `···` vocabulary. **The last Material frames shipped 2026-09-18** — every field a `TendrilField`, the Add dialogs as sheets, the date picker's headline tamed, ellipsis everywhere, a sheet's title at `pageTitle`, a slide-over's content under the caption row. **The design layer shipped 2026-09-18** — D4–D8: the light hover perceptible, the Road Map's edges at full alpha, two idle tokens gone, one radius family through Material's shapes (rule 17), keyboard focus visible. **L9 + L10 shipped 2026-09-17** — the canvas's cards under a pointer: *Fit* and `+` in the bar, a double-click on the ground with the discard rule, the badges on hover or selection; **`EscapeBackInput` deleted** — the runtime dispatches Escape itself, the app's input had doubled every back. **L7 + L8 shipped 2026-09-17** — the quick-add strip on the find bar's field with the chips inline, the popup on the switcher's field, the tray ≤ 30 % of the pane, the Week header's minutes below a 90 dp lane. **L6 shipped 2026-09-17** — the switcher is a centred card on a wide window (Recents, sections, chords, a footer), the desktop's text field is `TendrilField` (the AI key, the sync passphrase, the find bar), the F1 card and the switcher share `CentredCard`. **L5 shipped 2026-09-17** — the window is borderless on the JetBrains Runtime (the bar is the title bar), every tab wears one bar, the time grid has Notion Calendar's proportions; the toolchain is Android Studio's JBR from this PR on. **The desktop audit ran 2026-09-17**: three full passes over every surface — `docs/critiques/desktop-layout-full.md`, `desktop-type-full.md`, `desktop-function-full.md`; the disposition in `tendril-spec.md` §0.10 item 22. **The audit's fixes shipped 2026-09-17**: menus, rows, the type classes and their audit rules, the copy, the find gate, the F1 card, the habit pane (`docs/critiques/desktop-audit-fixes.md`). **The Month grid shipped 2026-09-17** (L4; `docs/critiques/month-grid-mock.md`, `month-grid-function.md`). Next: item 22's Med/Low list — L5 (the Calendar's chrome) first.*
