...
| # | Item | Ruling | Note |
|---|---|---|---|
| DB1–DB5 | Relation, rollup, formula, summary/explain, computed grouping | **Build** | Reversal of §5.4's original deferral, already recorded 2026-09-06. Zero schema change: relation edges encode into the existing `PropertyValue.value`, rollups compute on read, the expression lives in `Property.config`. Gated on Milestone 0 reaching **every** device first — see the precondition below. |
