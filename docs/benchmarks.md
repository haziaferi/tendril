...
| 16 | **Canvas depth** — nested canvases, frames/sections, mind-map layout, Canvas templates | Heptabase, Miro, Storyflow | `canvas_nodes.kind = CANVAS`, a `frame` node | Canvas toolbar | *Mind-map layout done 2026-09-20 — the mind-map pass: a parent link on the node, a structure per board and per subtree (Free · Map · Right · Down) applied by Tidy, three levels, following frames, folding; Xmind, Mindomo and Freeplane measured (`docs/critiques/mind-map-grounds.md`); `tendril-spec.md` §3.7* | *Checked 2026-09-13 (step 8c):* a nested canvas **is** a page-embed of a Canvas page (no new entity); the mind-map layout is the outline block's (§0.6.2). Frames/sections and canvas templates → §0.10 item 15 — *built 2026-09-18 (frames, templates); collapse recorded.* |
...
| 10 | **Cross-block selection and undo** | Notion | A real gap, but the editor's — one `BasicTextField` per block — not the layout's | high | **Done 2026-09-19** — a block selection beside the text selection (Esc, Shift+↑↓, a marquee, a long press on the phone) with Delete / Ctrl+C/X/V/D / a group drag, and a block undo over the block operations; `tendril-spec.md` §3.1.1, `docs/critiques/block-selection-mock.md`, `-function.md` |
