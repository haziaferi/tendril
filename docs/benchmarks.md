...
| **14h — the shelf** — *·1 done 2026-09-16*: as planned, with the openers decided on `docs/mockups/shelf.html` (the page's `···` → *Show beside ▸*, the tree row's *Open beside* and Ctrl+click; not the Road Map's nodes), the page's own bar as the shelf header, the graph over the full graph's neighbourhood on the shared canvas, the Journal stored as a kind, a 45 % clamp, Ctrl+Shift+\ (`tendril-spec.md` §3.1). *·2 the small things next — closes the pass.* | A second pane beside the open page on a wide window: another page, the Road Map's local graph around this page, or today's Journal (B§13.6 #4). Opened from a page's `···` ("Open beside") and from the Road Map; closed with its own ×; width remembered like the tree's. | **Yes** (2026-09-13). Only Obsidian pairs graph and page; nobody pairs Journal-today with the page being written. |
