...
| **14g — theme on desktop** | The picker in the desktop Settings pane (mode, colour, typeface — all four colours and the Serif face are shared already), stored in `KeyValueStore`. | **System / Light / Dark in Settings, default System.** *·1 the model done 2026-09-16* (§13.7 built: ten registers solved to floors, the theme in the store on both platforms, one shared section; `tendril-spec.md` §2.3); *·2 the token map* (§13.8.3) and *·3 the ladder* (§13.8.1, v20) next. |
