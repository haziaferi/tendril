...
22. **The desktop audit (2026-09-17)** — ***the eight High fixed 2026-09-17 (the audit's fixes PR; ***L4 fixed 2026-09-17 — the Month grid PR (`docs/critiques/month-grid-function.md`: rows = pane / 5, chips 22 px at a 24 px pitch, four a cell at the user's window, beside Notion's 124 / 28 / 34);***  D1–D3 of `docs/critiques/desktop-design-layer.md` fixed with them; `docs/critiques/desktop-audit-fixes.md` re-measures: menus 49 → 30 px, Day rows 65 → 30, Agenda 52–61 → 30, Table 50 → 34, raw roles 196 → 0, three annotated sites). The design-layer pass's D4–D8 join the Med/Low list below.*** Three full `design-critique-plus` passes over every desktop
    ...
    pane is a stub. **Med / Low, recorded here by the reports' row ids, never dropped:** ***L5 fixed 2026-09-17 — the Calendar's chrome PR (`docs/critiques/calendar-chrome-function.md`: one 52 dp bar on every tab, the bar the window's title bar; L9's Tasks half with it);*** ***L6 fixed 2026-09-17 — the quick switcher PR (`docs/critiques/quick-switcher-function.md`: a centred card, a 36 dp field, rows at the profile's height, Recents; the AI key and the desktop passphrase on the same field);*** L6b the remaining nineteen Material `OutlinedTextField`s (the dialogs' and sheets' name fields) onto `TendrilField` — a sweep if wanted; ***L7 and L8 fixed 2026-09-17 — the quick-add strip PR (`docs/critiques/quick-add-strip-function.md`: the strip 44 dp on the find bar's field with the chips inline, the popup on the 36 dp field; the tray ≤ 30 % of the pane, the header's minutes below a 90 dp lane);*** L7b the parsed tokens highlighted inline instead of chips (TickTick, Fantastical) — offered, not taken; L9 the FABs on Tasks and the
    canvas; L10 canvas cards' *tap* caption, always-on delete disc, a card past the edge; L11 submenus
    ...
    overflow (`FlowRow`); L15 the Add task dialog is the one non-slide-over modal; T3 Material's
    16/24/32 through fields, the Add dialog's title and the date picker; T4 titles clip without
    ellipsis (an audit rule for `maxLines = 1`); T5 the slide-over's header takes `pageTitle`; T6
    (no change — recorded); F4 Delete view asks nothing; F5 Configure / Delete view only by a second
    click on the active chip; F6 the Board's empty state has no verb; F7 the New view sheet is two
    controls; F8 the page's and the database's `···` disagree (*Trash…* vs *Move to Trash*; no *Show on
    Road Map* on a database); F9 the label sheet lists nothing until typed; F10 *Someday (2)* reads as a
    header; F11 the hover card joined a folded map's lines; F12 the Reminders presets (with L14); F13
    the row menu's icon alignment; F14 the neighbourhood's first-draw overlap; F15 a black frame on
