...
23. **The phone catch-up and the phone audit (2026-09-18)** — *(**Decided 2026-09-18** — two passes, two PRs: the catch-up of the phone halves pending since #82 (`docs/critiques/phone-catch-up.md`), then a measured `design-critique-plus` audit of every phone surface beside TickTick, Todoist, Obsidian, Notion and Notion Calendar on the same phone; every High into one follow-up fix PR; Med/Low here; the phone's data kept.)* **Catch-up findings:** ***P1 fixed 2026-09-18 with the catch-up — the launch crash (`LabelFilterRow`'s negative padding under Touch, since 14h·2);*** **P2** (High) a `TendrilSheet` on the phone does not scroll — with the keyboard up the Add task sheet's rows overlap; **P3** (Med) the Urgency submenu covers its parent on a 360 dp window — under Touch a submenu opens below its item; **P4** (Med) the Pages list's DPAD ↓ lands on the label chip, not the list's cursor; **P5** (Med) the canvas page has no `···` on the phone; **P6** (Med) task rows' meta is raw ISO and wraps; **P7** (Low) *Opens on*'s *Agenda* chip and *Restore backup* wrap; **P8** (Low) the Day view's preview chips start at the screen's edge; **P9** (Low) the find bar's hints name keys to a finger; **P10** (Low) the Day header's Italian order (*venerdì, settembre 18*); **P11** (process) nothing in the stand-in composes a layout — a Compose UI test of the Pages tab with a label is the fix PR's call. The phone audit's findings join this list when it runs.
...
- **Under a pointer** *(**Amended 2026-09-15 — B§13.4 14d, B§13.6 #1.** Three behaviours that
  ...
  **selects** it (a 2 dp accent ring; the badges — delete disc, link handle — show on hover or
  ...
  one ring and ↑↓ continue from where Tab landed.)*
