...
22. **The desktop audit (2026-09-17)** — ***the eight High fixed 2026-09-17 (the audit's fixes PR; L4 the Month grid is its own PR next; D1–D3 of `docs/critiques/desktop-design-layer.md` fixed with them; `docs/critiques/desktop-audit-fixes.md` re-measures: menus 49 → 30 px, Day rows 65 → 30, Agenda 52–61 → 30, Table 50 → 34, raw roles 196 → 0, three annotated sites). The design-layer pass's D4–D8 join the Med/Low list below.*** Three full `design-critique-plus` passes over every desktop
    ...
    Month is the phone's dot grid (titled cells on a wide window; the dots PR folds in); T1 two type
    ...
    pane is a stub. **Med / Low, recorded here by the reports' row ids, never dropped:** L5 the
    Calendar's 184 px of chrome (the view picker into the bar); L6 the switcher as a centred card with
    a 36 dp field; L7 the quick-add strip's 66 px field (the find bar's frame); L8 the tray unclamped
