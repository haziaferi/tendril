...
*(**Amended 2026-09-20 — the card's width.** The user: the boxes shorter by default, growing with the text, wrapping past a length — "to avoid overlap when there are many short text boxes" — and a 1 px border around the cards. The rule was **chosen by `meta-optimizer`** over the user's own texts against Xmind measured live (`docs/critiques/card-width-optimizer.md`): a card is **as wide as its text asks, between 60 and 220 dp, 8 dp of padding a side**, then it wraps (`cardWidth`, `CANVAS_CARD_CHAR` 8.5 dp a character — 0.6 em of the 14 sp text, the estimate every box reader shares); fitness 0.847 against 0.466 for the fixed 200 strip (its overlap score 0.233 was the complaint, measured) and 0.761 for widths snapped to steps. Every strip takes it — free, branch and page cards (a page card is its title's width; `CanvasTree` carries the titles) and the outline's map; the root keeps its 220 × 64 box; a leaf its line. An empty card is its caption's width (*Empty card* — the card is the verb). **A hand may set a card's width** (Obsidian's): a handle straddling the **left** edge's middle on a selected card under a pointer — the right edge stays, the `width` column is written (read as a hand's only when it is neither 0 nor the entity's old 180 — `handWidth`), the text keeps deciding the height. Every strip wears **a 1 dp `outline` hairline**; the card is a `Box`, not a `Surface`, so its badges may **straddle the right edge** (the frame's handle straddles its corner the same way) instead of reserving a 28 dp column that was half of a short card. The phone's `···` gains *Tidy* and *Fit every card* (its bar holds only `···`; Fit had been missing on the phone).)*
