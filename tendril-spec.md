...
24. **Small things IV — Lows seen on later walks (opened 2026-09-18, item 7's walk; `docs/critiques/canvas-embed-measured.md`).** **S1** the row page's strip shows a Date cell's value as ISO (*Read on 2026-09-19*) on both platforms — the Table's cell format; F·P4's *ven 25* has not reached the database's date cells. **S2** the *Insert block* and *Block actions* slide-overs list rows at ≈ 47 px on the desktop against the profile's 29 (Material rows outside the type-class audit's reach). **S3** a text field's right-click on the desktop opens Compose's default context menu — white on a dark register, the system's Italian labels — with *Block actions…* appended; it wants the register's colours. **S4** on the phone a swipe that starts in the ≈ 40 dp band between the page's last item and the bottom bar does not scroll (the band is an inset's padding outside the list). ***Assessed 2026-09-20 against the build (every critique's *Recorded* list and this item, grep-verified):*** **obsolete, struck** — the phone's missing *Fit* (the bar's *Fit* is unconditional since the mind-map pass), the Postpone chips' wrap and the weekday initials (item 23's Lows), the Merged list's plain checkbox on a counting habit (`HabitRow` draws `HabitCheck`), the Month's dots (the Month grid PR), F·P5 (no path, twice). **Kept, cheap — one PR next:** S1–S4 above; **S5** the page bar shows a Journal day's raw `journal/…` title where the tree, the shelf and the switcher show the date (the one L12 site missed); **S6** the Journal's Today strip checks a counting habit with a plain `Checkbox` (`JournalTodayStrip.kt`) — `HabitCheck` with the amount, as the Habits and Merged rows; **S7** D9 — Tab stops on a tree row's hover-revealed `···` while it is invisible. **Kept, each its own PR when wanted:** **S8** the mind map's sibling reorder by drag (order is creation order; Xmind's); **S9** Timeline and Fishbone (grounded and decided, deferred until Down has been used); **S10** an edit-habit sheet — no field of a habit is editable after creation, the largest gap on the list; **S11** the block selection's Touch gutter (16 dp before a paragraph is thin for a thumb — a drag handle or a wider gutter); **S12** collapsible frames (decided *not now* after item 15; still valid); **S13** the database hue slider and card colours — the token map's last open slot, on which the Road Map's database fill and JSON Canvas colours wait. **Notes, not work:** a Down child's relationship crossing the tree (the geometry's — the outer sides are honoured), nested frames drawn alike, the marquee's no-scroll rule, the undo stack per page open, the strip's fast-typing loss (unreproduced), T6 and F15, `.canvas` import (no importer to extend), `pageTitle` at two line heights (T·P3), B§13.6's tabs (no), a wider canvas card (the strip answered the proportion).
...
17. ~~**An agent over the export, and a local model** — B§6 #18's other two halves, kept out of
    ...
    `docs/agent-over-export.md`: which export carries what (the Markdown zip is the one to hand
