...
| 2026-09-19 (item 4 — the check-in, decided in part) | §0.10 item 4 mocked and critiqued (`docs/mockups/check-in.html`, `docs/critiques/check-in-mock.md` — Daylio's shape [Assumed], Notion's journal *Mood* select [Verified] as the setup it improves on). **Decided:** the check-in lives on **the Journal's day page, above the Today strip; past days too, never a future one** — the words are the page under it. **Deferred** (the user, 2026-09-19): what one tap logs (mood and energy, five steps each, or mood alone) and the month view (the sheet of coloured discs). **Not built**: no schema, no code; the mock stands as the shape to build from. | §0.10 |
...
4. Whether a one-tap **mood/energy check-in** joins the human layer, and when — it is not a
   habit. *Mocked 2026-09-19 (`docs/mockups/check-in.html`, `docs/critiques/check-in-mock.md`). **Decided:** its home is the Journal's day page, above the Today strip — past days too, never a future one; the page's blocks are the words, so no note field. **Deferred:** the scales (mood + energy at five steps each, or mood alone) and the month view (a sheet of coloured discs by the day's latest mood, no count or average); schema v22 (`check_ins`: uid, date, at, mood, energy, deletedAt — the completions' tombstone rule) is the shape when built. Not built.*
...
- **The shell** *(**Amended 2026-09-14 — B§13.4 14a.** The five destinations, the running timer
  ...
  a right hairline, 52×30 pills and 11.5 sp labels, the timer at its foot (▶ title, tabular
